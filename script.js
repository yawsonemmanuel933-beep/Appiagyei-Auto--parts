...
const $itemImageRemove = document.getElementById('itemImageRemove');
let currentItemImage = ''; // base64 data URL of the selected product image
              ...
              ${group.variants.map(product => {
                ...
                return `
                  <div class="prod-variant-card">
                    ${product.image ? `<img class="prod-variant-thumb" src="${product.image}" alt="${group.brand} ${group.carType}" />` : `<span class="prod-variant-thumb prod-variant-thumb-empty">📷</span>`}
                ...
                return `
                  <div class="inv-variant-card">
                    <div class="inv-variant-head">
                      ${product.image ? `<img class="inv-variant-thumb" src="${product.image}" alt="${product.brand} ${product.carType}" />` : `<span class="inv-variant-thumb inv-variant-thumb-empty">📷</span>`}
...
function showAddCustomerModal() {
  if ($addCustomerModal) {
    ...
    if ($salesCustomerName) $salesCustomerName.focus();
  }
}
...
function handleInventoryAction(action, productId) {
  ...
  if (action === 'increase-stock') {
    ...
    // Load the product image into the uploader
...
function handleItemImageChange(file) {
  if (!file) return;
  ...
  reader.onload = (ev) => {
    const img = new Image();
    img.onload = () => {
      // Resize/compress client-side so images stay small in storage & sync
      const MAX = 500;
      let width = img.width;
      ...
      currentItemImage = canvas.toDataURL('image/jpeg', 0.72);
      showItemImagePreview();
    };
    img.onerror = () => alert('Could not read the selected image. Please try another file.');
    img.src = ev.target.result;
  };
  reader.onerror = () => alert('Could not read the selected file.');
  reader.readAsDataURL(file);
}
function showItemImagePreview() {
  if (!currentItemImage) {
    if ($itemImagePreview) $itemImagePreview.classList.remove('has-image');
    if ($itemImageRemove) $itemImageRemove.style.display = 'none';
    return;
  }
  if ($itemImagePreviewImg) $itemImagePreviewImg.src = currentItemImage;
  if ($itemImagePreview) $itemImagePreview.classList.add('has-image');
  if ($itemImageRemove) $itemImageRemove.style.display = '';
}
...
function addOrUpdateStock(event) {
  ...
  if (editingItemId !== null) {
    ...
    if (existing) {
      ...
      existing.lowThreshold = lowThreshold;
      existing.image = currentItemImage;
  ...
  // Always create a new product entry (do not merge with existing by carType)
  const id = inventory.length ? Math.max(...inventory.map((item) => item.id)) + 1 : 1;
  inventory.push({ id, brand, carType, year, sidePart, price, cost, quantity, lowThreshold, image: currentItemImage });
...
function setupEventListeners() {
  ...
  if ($addCustomerModal) {
    ...
    $addCustomerModal.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') hideAddCustomerModal();
    });
  }
  ...
  if ($itemImage) {
    $itemImage.addEventListener('change', (e) => handleItemImageChange(e.target.files && e.target.files[0]));
  }
  if ($itemImageRemove) {
    $itemImageRemove.addEventListener('click', (e) => {
      e.preventDefault();
      resetItemImage();
    });
  }
  ...
  if ($credentialsModal) {
    ...
    $credentialsModal.addEventListener('keydown', (e) => {
  ...
  if ($companyNameEditor) {
    ...
    $companyNameEditor.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') $companyNameEditor.style.display = 'none';
    });
  }
  if ($companyNameInput) {
